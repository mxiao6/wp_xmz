...
class mts_googleplus_Widget extends WP_Widget {
    ...
    {
        $widget_ops = array('classname' => 'mts_googleplus', 'description' => __('Adds a beautiful Google Plus badge widget.','bd'));
        $control_ops = array('id_base' => 'google-badge-box');
        $this->WP_Widget('google-badge-box', theme_name . __(' - Google Plus Badge Box','bd'), $widget_ops, $control_ops);
    }
    ...
    {
        $defaults = array('title' => __('Find us on Google Plus','bd'), 'page_url' => '', 'width' => '280', 'color_scheme' => 'light', 'gp_layout' => 'portrait', 'page_type' => 'profile', 'cover_photo' => true, 'tagline' => true);
        $instance = wp_parse_args((array) $instance, $defaults); ?>

        <p>
            <label for="<?php echo $this->get_field_id('page_type'); ?>"><?php _e('Page type','bd'); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Google+ Page URL','bd'); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('color_scheme'); ?>"><?php _e('Color Scheme','bd'); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('gp_layout'); ?>"><?php _e('Layout','bd'); ?>:</label>
        ...
        <p>
            <b><?php _e('Portrait Layout Settings','bd'); ?></b>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id('cover_photo'); ?>"><?php _e('Cover Photo','bd'); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id('tagline'); ?>"><?php _e('Tagline','bd'); ?></label>
        </p>
    <?php
    }
}
