    ...
    if(($bd_data['post_meta'] && empty($bd_hide_post_meta)) || $bd_hide_post_meta == 'yes'):
        ...
        <?php
            ...
            if( $bd_data['post_meta_author'] ) {
                ?>
                <span>
                        <?php _e('Posted by : ','bd'); the_author_posts_link(); ?>
                </span>
                <?php if ( get_the_author_meta( 'google' ) ){ ?>
            ...
            if( $bd_data['post_meta_comments'] ) {
                if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
                    ?>
                    <span class="comments-link"><?php comments_popup_link( __( 'Leave a comment', 'bd' ), __( '1 Comment', 'bd' ), __( '% Comments', 'bd' ) ); ?></span>
                <?php
                }
            }
            ...
            if ( is_page() ) {
                ...
                if( $bd_data['post_meta_cats'] ){
                    echo "<span class='post_meta_cats'>" .__('In','bd'). "\n";
                    the_category(', ');
                    echo "</span>\n";
                }
			}
            ...
            edit_post_link( __( 'Edit', 'bd' ), '<span class="edit-link">', '</span>' );
...
function getPostLikeLink($post_id){
    ...
    if(hasAlreadyVoted($post_id))
        $output .= ' <span title="'.__('I like this article', $themename).'" class="qtip like alreadyvoted"><i class="icon-heart"></i></span>';
    else
        $output .= '<a href="#" data-post_id="'.$post_id.'">
					<span  title="'.__('I like this article', $themename).'"class="qtip like"><i class="icon-heart"></i></span>
				</a>';
    if($vote_count)
